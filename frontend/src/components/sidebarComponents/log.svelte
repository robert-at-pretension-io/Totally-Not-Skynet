<script lang="ts">
  ...
  const unsubscribe = systemStateStore.subscribe((value: SystemState) => {
    responses = value.execution_context.responses;
    prompts = value.execution_context.prompts;
    topological_order = value.execution_context.topological_order;
    current_node = value.execution_context.current_node;
    local_variables = value.execution_context.local_variables;
  });
  ...
  $: if (current_node != null) {
    ...
    if (topological_order.indexOf(current_node) > 0) {
      ...
      let previous_node =
        topological_order[topological_order.indexOf(current_node) - 1];
              ...
              if (!local_variables.has(variable)) {
                setLocalExecutionVariable(
                  ...
                  if (current_node != null) {
                    ...
                    if (prompt != undefined) {
                      let this_prompt_text = addVariablesToPrompt(
                        prompt,
                        value
                      );
