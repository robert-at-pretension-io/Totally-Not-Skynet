<script lang="ts">
  ...
  $: {
    mainObject =
      $systemStateStore.selectedAction || $systemStateStore.selectedProcess;
  }
...
<div class="json-editor">
  {#if mainObject !== null && Object.keys(mainObject).length > 0}
    {#each Object.entries(mainObject) as [key, value], index (index)}
      {#if key !== "_id" && key != "graph"}
        <!-- Skip entities with key of "_id" -->
